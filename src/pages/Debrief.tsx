...
import { CheckCircle2, XCircle, ChevronDown, Download, Share2, BookOpen, TrendingUp, Award, AlertCircle } from "lucide-react";
...
const Debrief = () => {
  ...
  const loadDebriefData = async () => {
    try {
      ...
      const {
        data: run,
        error: runError
      } = await supabase.from("simulation_runs").select(`
          id,
          student_id,
          score_json,
          created_at,
      ...
      const timeTaken = run.start_at && run.end_at ? Math.floor((new Date(run.end_at).getTime() - new Date(run.start_at).getTime()) / 1000) : 0;

      // Process rubric data
      const rubric = scoreData?.rubric || [];
      const missedItems = rubric.flatMap((section: any) => section.items?.filter((item: any) => !item.achieved).map((item: any) => ({
        id: item.id,
        text: item.text,
      ...
      const learningPearls = clinicalData?.learningPearls || [{
        ...
        ref: "NCISM CBDC Standards"
      }];
      const stepwiseReasoning = scoreData?.reasoning || ["Review the clinical approach for this case type", "Practice systematic history taking and examination", "Study relevant Ayurvedic principles and modern correlations"];

      // Get remediation MCQs
      const {
        data: mcqsData
      } = await supabase.from("mcqs").select("question_json").eq("case_id", run.assignments?.case_id).limit(3);
      ...
      const grade = scoreData?.grade || (scoreData?.percent >= 85 ? "Distinction" : scoreData?.percent >= 70 ? "Pass" : scoreData?.percent >= 50 ? "Borderline" : "Fail");
  ...
  return <div className="min-h-screen bg-background pb-20">
      <TopMicroHeader title="Performance Debrief" subtitle="रोगी परीक्षण फीडबैक" onBack={() => navigate("/student")} />
      ...
      <div className="p-4 space-y-4">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            {debriefData.rubric.map((section, idx) => <Collapsible key={idx} open={openSections[section.section]} onOpenChange={open => setOpenSections({
            ...openSections,
            [section.section]: open
                ...
                <Card className="p-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 text-left">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <Progress value={section.score / section.max * 100} className="h-2 flex-1" />
                  ...
                  <CollapsibleContent className="mt-3 space-y-2">
                    {section.items.map((item: any, itemIdx: number) => <div key={itemIdx} className="flex items-start gap-2 text-xs">
                        {item.achieved ? <CheckCircle2 className="h-4 w-4 text-green-600 shrink-0 mt-0.5" /> : <XCircle className="h-4 w-4 text-red-500 shrink-0 mt-0.5" />}
                        <div className="flex-1">
                          <p className={item.achieved ? "text-foreground" : "text-muted-foreground"}>
                            {item.text}
                          </p>
                          {item.evidence && <p className="text-muted-foreground italic mt-1">{item.evidence}</p>}
                          {!item.achieved && item.tip && <p className="text-yellow-600 mt-1">💡 {item.tip}</p>}
                        </div>
                      </div>)}
                  </CollapsibleContent>
                </Card>
        ...
        {/* Missed Checklist */}
        {debriefData.missedChecklist.length > 0 && <Card className="p-4 border-yellow-200 bg-yellow-50/50">
            ...
            <div className="space-y-2">
              {debriefData.missedChecklist.map((item: any, idx: number) => <Card key={idx} className="p-3 bg-white">
        ...
        <Card className="p-4">
          ...
          <ol className="space-y-2">
            {debriefData.stepwiseReasoning.map((step, idx) => <li key={idx} className="text-sm text-muted-foreground">
                {step}
              </li>)}
          </ol>
        </Card>
        ...
        <Card className="p-4 bg-blue-50/50 border-blue-200">
          ...
          <div className="space-y-2">
            {debriefData.learningPearls.map((pearl, idx) => <div key={idx} className="text-sm">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-4">
            {debriefData.mcqs.map((mcq, mcqIdx) => <Card key={mcqIdx} className="p-4 bg-accent/5">
                ...
                return <Button key={choiceIdx} variant={isSelected ? "default" : "outline"} className={`w-full justify-start text-left h-auto py-2 ${showAnswer && isCorrect ? "border-green-500 bg-green-50" : ""} ${showAnswer && isSelected && !isCorrect ? "border-red-500 bg-red-50" : ""}`} onClick={() => handleMCQAnswer(mcqIdx, choiceIdx)} disabled={showAnswer}>
                ...
                {selectedMCQ[mcqIdx] !== undefined && <p className="text-xs text-muted-foreground mt-3 p-2 bg-blue-50 rounded">
                    {mcq.explanation}
                  </p>}
              </Card>)}
          </div>
